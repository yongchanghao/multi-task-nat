        ...
        **kwargs
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores):
            ...
            if self.normalize_scores:
               ...
               #  cov = torch.zeros_like(eos_scores) 
                cov = attn_clone.clone().sum(dim=-1)
                cov = torch.min(torch.ones_like(cov), cov + 1e-9)
                cov = torch.log(cov).sum(dim=-1)
                # print(cov.mul(self.cov_penalty))
                assert torch.equal(cov.mul(0.0), torch.zeros_like(cov))
                eos_scores += cov.mul(self.cov_penalty)
