...
def main(args):
    ...
    num_sentences = 0
    has_target = True
    n_samples = len(task.datasets[args.gen_subset].src_sizes)
    with progress_bar.build_progress_bar(args, itr) as t:
        wps_meter = TimeMeter()

        if args.decoding_path is not None:
            src_sents = [[] for _ in range(n_samples)]
            tgt_sents = [[] for _ in range(n_samples)]
            hyp_sents = [[] for _ in range(n_samples)]
    ...
    if args.decoding_path is not None:
        with open(os.path.join(args.decoding_path, 'source.txt'), 'w', encoding="utf8") as f:
        ...
        with open(os.path.join(args.decoding_path, 'target.txt'), 'w', encoding="utf8") as f:
        ...
        with open(os.path.join(args.decoding_path, 'decoding.txt'), 'w', encoding="utf8") as f:
